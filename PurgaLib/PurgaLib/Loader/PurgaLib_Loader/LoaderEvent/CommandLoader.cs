...
namespace PurgaLib.Loader.PurgaLib_Loader.LoaderEvent
{
    public static class CommandLoader
    {
        ...
        public static void RegisterCommands(object plugin)
        {
            ...
            {
                Logged.Error($"{LoggerPrefix} Error reading assembly types: {rtle}");
                types = rtle.Types?.Where(t => t != null).ToArray() ?? Array.Empty<Type>();
            }
            catch (Exception ex)
            {
                Logged.Error($"{LoggerPrefix} Error reading assembly types: {ex}");
                return;
            }
            ...
            foreach (Type type in types)
            {
                ...
                foreach (var attrData in type.GetCustomAttributesData())
                {
                    ...
                    {
                        Logged.Error($"{LoggerPrefix} CommandHandlerAttribute argument is not a Type for {type.FullName}");
                        continue;
                    }
                    ...
                    if (TryRegister(type, handlerType, out ICommand created, pluginName, out string err))
                    {
                        ...
                        Logged.SendRaw($"{LoggerPrefix} Registered command '{created.Command}' for plugin '{pluginName}'", ConsoleColor.Green);
                    }
                    else
                    {
                        Logged.Error($"{LoggerPrefix} Failed to register command {type.Name} : {err}");
                    }
                }
            }
        ...
        public static void UnregisterCommands(object plugin)
        {
            ...
            foreach (var entry in list)
            {
                ...
                if (handler == null)
                {
                    Logged.Error($"{LoggerPrefix} Could not find handler {entry.HandlerType?.Name} to unregister.");
                    continue;
                }
                ...
                {
                    Logged.Error($"{LoggerPrefix} Error unregistering command {entry.Command.Command}: {ex}");
                }
            }
        ...
        private static CommandHandler ResolveHandlerSafely(Type handlerType)
        {
            ...
            {
                Logged.Error($"{LoggerPrefix} Error resolving handler {handlerType.Name}: {ex}");
                return null;
            }
        }
    }
