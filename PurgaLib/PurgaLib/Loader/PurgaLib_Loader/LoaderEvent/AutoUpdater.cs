...
using PurgaLib.Loader.PurgaLib_Loader.GithHubAPI;
using System;
...
namespace PurgaLib.Loader.PurgaLib_Loader.LoaderEvent
{
    internal sealed class PurgaUpdater
    {
        ...
        private string Folder =>
            Directory.Exists(Path.Combine(PathManager.Plugins.FullName, "global"))
                ? "global"
            ...
            try
            {
                ...
                ProcessStartInfo info = new()
                {
                    ...
                    Arguments =
                        ...
                        $"--appdata \"{PathManager.AppData.FullName}\""
                };
                ...
                proc.OutputDataReceived += (_, e) =>
                {
                    if (!string.IsNullOrWhiteSpace(e.Data))
                        Logged.Info($"[Updater] {e.Data}");
                };

                proc.ErrorDataReceived += (_, e) =>
                {
                    if (!string.IsNullOrWhiteSpace(e.Data))
                        Logged.Error($"[Updater] {e.Data}");
                };
                ...
                if (proc.ExitCode == 0)
                {
                    Logged.Warn("PurgaLib updated. Server will restart next round.");
