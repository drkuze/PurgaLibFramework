...
namespace PurgaLib.API.Features
{
    public sealed class AutoWhitelist : PActor
    {
        ...
        public void Enable()
        {
            if (_isActive) return;
            _isActive = true;
            Logged.Info("Server whitelist mode enabled");
        ...
        public void Disable()
        {
            if (!_isActive) return;
            _isActive = false;
            Logged.Info("Server whitelist mode disabled");
        }
        ...
        private void KickNonWhitelistedPlayers()
        {
            foreach (var player in Player.List)
            {
                if (!_whitelist.Contains(player.UserId))
                {
                    ...
                    Logged.Info($"Kicked {player.Nickname} ({player.UserId}) because server is whitelisted");
                }
            }
        }
        ...
        public void AddToWhitelist(Player player)
        {
            if (_whitelist.Add(player.UserId))
                Logged.Info($"Added {player.Nickname} ({player.UserId}) to whitelist");
        }

        public void RemoveFromWhitelist(Player player)
        {
            if (_whitelist.Remove(player.UserId))
                Logged.Info($"Removed {player.Nickname} ({player.UserId}) from whitelist");
        }
        ...
        protected override void Tick()
        {
            ...
            foreach (var player in Player.List)
            {
                ...
                {
                    player.Kick(KickReason);
                    Logged.Info($"Kicked {player.Nickname} ({player.UserId}) during Tick check");
                }
            }
        }
